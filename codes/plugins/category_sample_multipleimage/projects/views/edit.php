<div class="wrap">
  ...
  <form id="project_edit_form" name="project_edit_form" method="post" action="<?php echo $this->base_url; ?>" enctype="multipart/form-data">
    ...
    <table class="form-table">
      <tbody>
        ...
        <?php
            ...
            $categories = $ProjectCategories->getCategories();
        ?>
    ...
    <p class="submit">
      ...
      <input type="hidden" value="<?php echo $row->ID; ?>" name="wa_data[id]" id="project_id">
...
<script language="JavaScript">
    jQuery(document).ready(function() {
        ...
        // START: slug generate
        jQuery('#project_edit_form #title').keyup(function() {
            var str = this.value;
            jQuery.ajax({
                type: 'POST',
                url: ajaxurl,
                ...
                success: function(response){
                    jQuery('#project_edit_form #slug').val(response);
                }
            });
        });
        // END: slug generate
        ...
        // START: slug checking
        jQuery('#project_edit_form').submit(function(ev) {
            var slug = jQuery(this).find('#slug').val();
            var project_id = jQuery(this).find('#project_id').val();
            jQuery.ajax({
                type: 'POST',
                url: ajaxurl,
                data: {
