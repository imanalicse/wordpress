...
class Samples_Table extends WP_List_Table {
	...
	function prepare_items($s=null) {
		...
		global $wpdb;
		$orderby = (!empty($_REQUEST['orderby'])) ? $_REQUEST['orderby'] : 'ordering';
		$order = (!empty($_REQUEST['order'])) ? $_REQUEST['order'] : 'asc';

		if (strlen($s)>0) {
			$sql = 'SELECT * FROM '.$wpdb->prefix.'samples WHERE title LIKE "%'.$s.'%" ORDER BY ' . $orderby . ' ' . $order . '';
		}
		else {
			$sql = 'SELECT * FROM '.$wpdb->prefix.'samples ORDER BY ' . $orderby . ' ' . $order . '';
		}
