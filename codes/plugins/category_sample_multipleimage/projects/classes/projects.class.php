...
class Projects_Table extends WP_List_Table {
    ...
    function __construct() {
		...
		//Set parent defaults
		parent::__construct(array('singular' => 'Project', //singular name of the listed records
	...
	function column_catid($item) {
		if($item['catid']){ 		
			...
			$category = $ProjectCategories->getCategoryById($item['catid']);
			if(!empty($category)){
	...
	function column_published($item) {
		if ($item['published'] == 0) {
			return '<img src="'.plugins_url( 'projects' ).'/images/unpublish.png" alt="unpublished" />';
		}
		else {
			return '<img src="'.plugins_url( 'projects' ).'/images/publish.png" alt="published" />';
		} 
    ...
    function extra_tablenav( $which ) {
        ...
        $query = "SELECT * FROM ".$wpdb->prefix."project_categories WHERE published = 1";
	...
	function prepare_items() {
        ...
        $sql = 'SELECT * FROM '.$wpdb->prefix.'projects ' . $where . ' ORDER BY ' . $orderby . ' ' . $order . '';
