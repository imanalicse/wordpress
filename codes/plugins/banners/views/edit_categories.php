<div class="wrap">
  ...
  <form id="banner_category_edit_form" name="banner_category_edit_form" method="post" action="<?php echo $this->base_url; ?>" enctype="multipart/form-data">
    ...
    <table class="form-table">
      <tbody>
        ...
        <tr class="form-field form-required">
            ...
            <td><input type="text"id="slug" name="form_data[slug]" value="<?php echo $row->slug; ?>" /></td>
        </tr>
       <tr class="form-field">
    ...
    <p class="submit">
      ...
      <input type="hidden" value="<?php echo $row->ID; ?>" name="form_data[id]" id="banner_category_id">
    </p>
...
<script language="JavaScript">
    jQuery(document).ready(function() {
        ...
        // START: slug generate
        jQuery('#banner_category_edit_form #title').keyup(function() {
            var str = this.value;
            jQuery.ajax({
                type: 'POST',
                url: ajaxurl,
                ...
                success: function(response){
                    jQuery('#banner_category_edit_form #slug').val(response);
                }
            });
        });
        // END: slug generate
        ...
        // START: slug checking
        jQuery('#banner_category_edit_form').submit(function(ev) {
            var slug = jQuery(this).find('#slug').val();
            var banner_category_id = jQuery(this).find('#banner_category_id').val();
            jQuery.ajax({
                type: 'POST',
                url: ajaxurl,
                data: {
                    ...
                    "table_name":'banner_categories'
                },
                async: false,
                success: function(response){
                    if(response>0){
                        alert('Same banner category already exist. Please change.');
                        ev.preventDefault();
                    }else{
                        jQuery(this).submit();
                    }
